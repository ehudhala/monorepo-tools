...
SUBDIRECTORY_SED=${SUBDIRECTORY//-/\\-} TAB=$'\t' git filter-branch -f \
    --index-filter '
    ...
    --tag-name-filter 'cat' \
    --msg-filter 'sed "s/^/$SUBDIRECTORY_SED: /"' \
    --commit-filter 'echo -n "${GIT_COMMIT}," >> /tmp/commit_map; \
                     NEW=`git_commit_non_empty_tree "$@"`; \
                     echo "${NEW}" >> /tmp/commit_map; echo ${NEW}' \
    -- $REV_LIST_PARAMS
